...
public interface ChecklistEquipementRepository extends JpaRepository<ChecklistEquipement, Integer> {
    ...
    @Query("SELECT c FROM ChecklistEquipement c WHERE c.intervention.idIntervention IN :ids")
    List<ChecklistEquipement> findByInterventionIds(@Param("ids") List<Integer> ids);
}